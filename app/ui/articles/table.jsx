...
export default function Table({ totaldata, totalPages, unique_name }) {
  ...
  function dateCoverter(isoDateString) {
    const dateObject = new Date(isoDateString);

    const userLocale = navigator.language || "en-US"; // fallback to 'en-US' if no locale is available

    const formattedDateString = dateObject.toLocaleString(userLocale, {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
      hour: "numeric",
            ...
            {JSON.parse(totaldata).map((activity) => {
              return (
                <tr data-option={activity.id} key={activity.id}>
                  ...
                  <td className="px-5 p-2 border-r border-[#EBEBEB]">
                    {dateCoverter(activity.created_on)}
                  </td>
